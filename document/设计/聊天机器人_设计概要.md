...
|| 音乐界面刷新功能 | 音乐播放界面刷新后仍能保留音乐信息 |
| 机器人server | 实现与前端通信的能力 | 能够获取前端的请求，并给出响应 |
|| 解读用户query，提取关键字，分析意图 | 解析query，判断用户的query是否是音乐请求,查看能否从存储文件中找到该歌曲名 |
...
### 2.7 运行设计
![时序图](./img/sequence diagram.png)

### 2.8 算法实现图

![算法初步实现图](./img/pattern01.png)

![算法最终实现图](./img/algorithm.png)
...
### 4.1 性能
&emsp; 1.优化query解析算法逻辑以提升性能 
&emsp; 2.丰富话术模版数据pattern以提高比对处理的准确性和有效性 


### 4.2 可靠性
&emsp;1.优化项目时尽量不改变项目原有结构，以保证项目安全 
&emsp;2.将服务返回限定在闲聊与技能两者之间，保证返回内容的有效、有意义
