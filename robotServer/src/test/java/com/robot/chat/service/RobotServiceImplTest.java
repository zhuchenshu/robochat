...
public class RobotServiceImplTest extends ChatApplicationTests {
    ...
    public void getResponse() {
        ...
        Assertions.assertThat(back.getNlu().getDomain()).isEqualTo("music");
        Assertions.assertThat(back.getPayload().getMusic()).isNotNull();

        logger.info("测试输入闲聊接口时输出闲聊类型回复");
        String queryChat = "你好呀";
        back = robotService.getResponse(queryChat);
        Assertions.assertThat(back.getHeader().getSkillId()).isEqualTo(0);
        Assertions.assertThat(back.getNlu().getDomain()).isEqualTo("chat");
    }
}